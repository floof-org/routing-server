...
export const validate = {
    ...
    secretKey: v => {
        if (typeof v !== "string" || v.length > 48 ) {
            throw new Error("secretKey must be a string of 48 characters or less");
        }

        return v;
    },
    gamemode: v => {
        if (!["ffa", "tdm", "waves", "line", "maze", "mmo"].includes(v)) {
    ...
    biome: v => {
        v = +v;
        if (isNaN(v) || v < 0 || v > 9) {
            throw new Error("biome must be a number between 0 and 9");
        }
...
export default class Lobby {
     ...
     * @param {string} secret
     */
    addClient(client, userId, secret) {
        client.data.clientID = this.idManager.next();
        this.clients.set(client.data.clientID, client);
        this.ownerSocket.send(new Uint8Array([0x00, ...u16ToU8(client.data.clientID), ADMINS[secret] ? 1 : 0, ...stringToU8(userId)]));
