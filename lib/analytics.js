...
const validate = {
    ...
    tzOff: v => {
        // 1. Verify it is a valid number (and not NaN)
        if (typeof v !== 'number' || Number.isNaN(v)) {
            throw new Error("timezoneOffset must be a valid number");
        }

        // 2. Earth's time zones range from UTC-12 to UTC+14
        const MIN_OFFSET_HOURS = -12;
        const MAX_OFFSET_HOURS = 14;

        if (v < MIN_OFFSET_HOURS || v > MAX_OFFSET_HOURS) {
            throw new Error(`timezoneOffset must be between ${MIN_OFFSET_HOURS} and ${MAX_OFFSET_HOURS} hours`);
        }

        // 3. Check for valid increments (optional but highly recommended)
        // Time zones only exist in whole hours, half hours, or 45-min increments.
        // Multiplying by 4 checks if it's a multiple of 0.25.
        if ((v * 4) % 1 !== 0) {
            throw new Error("timezoneOffset must be in valid 15-minute (0.25 hour) increments");
        }
